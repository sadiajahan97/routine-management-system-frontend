    ...
    <div className='border border-neutral-b40 bg-white flex flex-col gap-8 p-4 rounded-[12px]'>
      ...
      <div className='flex flex-col items-start gap-3 pt-3'>
        <Button
          ...
          <FontAwesomeIcon className='h-auto text-neutral-b700 w-3.25' icon={faEye} />
          <div className='font-medium text-base text-neutral-b700'>Change View</div>
        </Button>
        {view === 'grid' ? (
          <GridView routines={routines} selectedId={routine.id} setRoutine={setRoutine} />
        ) : (
          <TimelineView routines={routines} selectedId={routine.id} setRoutine={setRoutine} />
        )}
      </div>
    </div>
  );
